...
def prepare_database():
    execute(
        """
        IF COL_LENGTH('products', 'photo') IS NULL
        ALTER TABLE products ADD photo NVARCHAR(300) NOT NULL DEFAULT N''
        """
    )
...
def load_products():
    ...
    if search:
        rows = fetch_all(
            """
            SELECT id_p, article_num, product, category, description, creator,
                   supplier, price, unit, stock_quantity, discount, photo
            FROM products
            WHERE product LIKE ? OR category LIKE ? OR supplier LIKE ?
            ORDER BY id_p
            """,
...
def create_product_card(row):
    ...
    Label(info_frame, text=f"Производитель: {manufacturer}", bg=bg, fg=fg, anchor="w").pack(fill="x")
    ...
    Label(info_frame, text=f"Количество на складе: {stock_quantity}", bg=bg, fg=fg, anchor="w").pack(fill="x")
...
def open_product_form(product_id=None):
    ...
    if product_id is not None:
        row = fetch_one(
            """
            SELECT article_num, product, category, description, creator, supplier,
                   price, unit, stock_quantity, discount, photo
            FROM products
            WHERE id_p=?
            """,
            [product_id],
        ...
        description_var.set(row[3])
        manufacturer_var.set(row[4])
        supplier_var.set(row[5])
        price_var.set(str(row[6]))
        unit_var.set(row[7])
        stock_var.set(str(row[8]))
        discount_var.set(str(row[9]))
...
def save_product(window, product_id, article_var, product_var, category_var, description_var, manufacturer_var, supplier_var, price_var, unit_var, stock_var, discount_var, photo_var):
    ...
    if article_num == "" or product == "" or category == "" or manufacturer == "" or supplier == "":
    ...
    if product_id is None:
        execute(
            ...
            INSERT INTO products (
                article_num, product, category, description, creator, supplier,
                price, unit, stock_quantity, discount, photo
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [article_num, product, category, description, manufacturer, supplier, price, unit, stock_quantity, discount, photo],
        )
    else:
        execute(
            """
            UPDATE products
            SET article_num=?, product=?, category=?, description=?, creator=?,
                supplier=?, price=?, unit=?, stock_quantity=?, discount=?, photo=?
            WHERE id_p=?
            """,
            [article_num, product, category, description, manufacturer, supplier, price, unit, stock_quantity, discount, photo, product_id],
        )
...
def delete_product():
    ...
    execute("DELETE FROM products WHERE id_p=?", [selected_product_id])
    load_products()
